...
declare -i dnssleep=150

. ./functions/login.incl
...
domCount=$( \
    ./bin/xpath ./download/gratisdns.xml '//td[@class="table-success"]' 2>&1 \
    | sed -E -e '1,2d' \
    | wc -l )
if [[ ! ${domCount} -eq 1 ]]
then
    fejlTekst=$( ./bin/xpath ./download/gratisdns.xml '//td[@class="table-danger"]/text()' 2>/dev/null )
...
./bin/xpath ./download/gratisdns.xml \
